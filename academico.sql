...
CREATE VIEW vw_facultad_asignatura AS
SELECT facultad.nombre as facultad, COUNT(asignatura.id) as cantidad
from facultad
left join programa on facultad.id = programa.idfacultad
left join malla ON programa.id = malla.idprograma 
left join asignatura ON malla.idasignatura = asignatura.id
...
SELECT * FROM vw_facultad_asignatura WHERE cantidad>=10;